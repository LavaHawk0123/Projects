...
class Traverse:
	...
	def __init__(self,x,y):
		...
		R = rospy.Rate(1)
		R.sleep()
		self.sub3 = rospy.Subscriber("/sensor/us_sensor_left",Range,self.get_us_left)
		self.sub4 = rospy.Subscriber("/sensor/us_sensor_right",Range,self.get_us_right)
		self.sub2 = rospy.Subscriber("/mybot/laser/scan",LaserScan,self.get_laser)
	...
	def get_us_left(self,req_left):
		self.left_obs = req_left.range
		if self.left_obs<1:
			self.Flag_left = True
		else:
			self.Flag_left = False

	def get_us_right(self,req_right):
		self.right_obs = req_right.range
		if self.right_obs<1:
...
#			self.Straight()
	...
	def get_laser(self,msg):
		...
		for i in range(0,fac):
			values.append(min(min(msg.ranges[Range:(Range+Iter)]),max_range))
			Range+=Iter
		#print(values)
		self.obs = {}
		for i in range(0,fac):
